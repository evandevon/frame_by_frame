...
- White balance control on supported cameras — automatic, or manual with a colour-temperature slider
- Additional image controls wherever the connected camera exposes them (brightness, contrast, saturation, optical/digital zoom, pan, tilt), with a one-click reset back to the camera's startup values
...
- Nudge a shaky frame — and everything baked onto it (ink, paint, mask, overlays, all move together as one unit) — into alignment with the onion-skin ghost of a neighbouring frame
- Drag directly on the frame while "Drag to Align" is active, or use the Zoom and Rotation sliders; each slider also has +/- buttons for nudging by a single step
- Zoom in first for room to nudge the frame without immediately revealing an empty edge; Rotation straightens a tilted shot
- Optional "Generate edge pixels" setting (applies project-wide) fills in whatever edge is left empty by a stabilize move
- Reset button restores the current frame's position, zoom, and rotation
...
- Draw on frames with a pen tool — adjustable colour, opacity, size, edge softness, and taper (a stroke can thin out toward the end like a marker running low, or stay a constant width)
- Ink is stored as a separate layer and does not alter the original photo
- Eraser tool
- **Glow Pen** — a soft, glowing stroke with a separate core colour and outer glow colour; adjustable size, opacity, and taper
...
- Manual erase/restore brush with adjustable size and softness, in Merge mode (adds to whatever's already erased/kept) or one-shot Isolate mode (resets the mask before this stroke, so only what you paint stays kept or erased and everything else flips)
- Rectangle, polygon, freehand erase tool — make a shape to erase or keep a region, with the same Merge/Isolate choice as the brush
...
- Add a track with the large "+ Track" button below the track list; remove one with the small × on that track's own header square — deleting is undo-safe, so an accidental click is just an Undo away
...
- Open the Edit Frame panel to adjust brightness/contrast, or use Stabilize to nudge a shaky frame into alignment with the onion-skin ghost of a neighbouring one.
- Open the Inking panel to draw on a frame, spray texture, smudge, glow, or paint in pixels from the frame before or after it with the Time-Clone Pen.
